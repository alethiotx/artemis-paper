...
def parse_filename(filepath: str) -> Tuple[str, str, str, str, str, str]:
    ...
    ----------
    filepath : str
        Path to prediction CSV (format: <kg>_<embedding>_<ct>_<threshold>_<pg>_<iter>.csv)
...
def prepare_plotting_data(metrics_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Melt to long format
    melted = metrics_df.melt(id_vars=['kg', 'embedding', 'ct', 'rf_prob', 'pathway_genes', 'name'])
...
def create_indication_plots(data: pd.DataFrame, results: Dict) -> None:
    ...
    for kg in sorted(results.keys()):
        for embedding in sorted(results[kg].keys()):
            # Specificity plot
            specificity_data = data[(data['kg'] == kg) & (data['embedding'] == embedding) & (data['name'] == 'specificity')]
            ...
            plot.save(str(indications_dir / f'specificity_{kg}_{embedding}.png'), dpi=300, width=10, height=5)
            print(f"  ✓ specificity_{kg}_{embedding}.png")
            
            # Sensitivity plot
            sensitivity_data = data[(data['kg'] == kg) & (data['embedding'] == embedding) & (data['name'] == 'sensitivity')]
            ...
            plot.save(str(indications_dir / f'sensitivity_{kg}_{embedding}.png'), dpi=300, width=10, height=5)
            print(f"  ✓ sensitivity_{kg}_{embedding}.png")


def create_kg_comparison_plot(data: pd.DataFrame) -> None:
    """
    Generate overall KG comparison plots, one per embedding.
    ...
    print("Generating KG comparison plots...")
    
    for embedding in sorted(data['embedding'].unique()):
        embedding_data = data[data['embedding'] == embedding]
        ...
        plot.save(str(kgs_dir / f'all_{embedding}.png'), dpi=300, width=10, height=6)
        print(f"  ✓ all_{embedding}.png")
...
def create_main_text_heatmap(results: Dict) -> None:
    ...
    data1 = results[config1['kg']][config1['embedding']][config1['ct']][config1['rf_threshold']][config1['pg_number']]
    ...
    data2 = results[config2['kg']][config2['embedding']][config2['ct']][config2['rf_threshold']][config2['pg_number']]
            ...
            for embedding in HEATMAP_EMBEDDINGS:
                ...
                for row_idx, kg in enumerate(HEATMAP_KGS):
                    for col_idx, ct in enumerate(HEATMAP_CTS):
                        ...
                        data = results[kg][embedding][ct][rf_threshold][pg_number]
                ...
                plt.tight_layout()
                output_path = heatmaps_dir / f'{rf_threshold}_{pg_number}_{embedding}.png'
                plt.savefig(output_path)
                plt.close()
                
                print(f"  ✓ {rf_threshold}_{pg_number}_{embedding}.png")
