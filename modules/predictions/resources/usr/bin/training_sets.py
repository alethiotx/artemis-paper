#!/usr/bin/env python
"""
Training Set Label Aggregation 

Aggregates training set binary labels across all indications, knowledge graphs,
...
Usage:
    training_sets.py <file1.csv> <file2.csv> ... <fileN.csv>

Arguments:
    files: CSV files with training labels (format: <ind>_<kg>_<ct>_<threshold>_<pg>_<iter>.csv)

Output:
    - all_training_sets.pickle: Nested dictionary of training labels by indication
"""
...
def load_training_labels(files: List[str]) -> Dict:
    """
    Load all training set files and organize by indication and parameters.
...
def main():
    ...
    print(f"Processing {len(files)} training set files...")
    
    # Load and organize training labels
    results = load_training_labels(files)
    ...
    output_path = Path('all_training_sets.pickle')
    
    with open(output_path, 'wb') as f:
        pickle.dump(results, f)
    
    print(f"✓ Saved aggregated training sets to {output_path}")
